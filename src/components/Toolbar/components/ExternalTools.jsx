...
const ExternalTools = defineComponent({
    ...
    setup() {
        ...
        const mockSimulation = () => {
            // 获取 ToggleMode 服务
            const toggleModeService = moduleStore.getModeler.get('toggleMode');
            // 切换到模拟模式
            toggleModeService.toggleMode();
        };
        let lintModule = null;
        ...
        const openShortcutKeysModel = () => {
            const renderDefault = () => (<div class="shortcut-keys-model">
          <p>Undo</p>
          <p>Ctrl + Z</p>
          <p>Redo</p>
          ...
          <p>S</p>
            ...
            const renderTemplateExternal = () => (<div class="shortcut-keys-model">
          <p>Replace Tool</p>
          <p>R</p>
          <p>Append anything</p>
          <p>A</p>
        ...
        const visibleListeners = computed(() => listeners.value.filter((i) => i.includes(listenerFilter.value)));
        const openEventsModel = () => {
            const eventBus = moduleStore.getModeler.get('eventBus');
            listenerFilter.value = '';
            listeners.value = Object.keys(eventBus._listeners).sort();
            eventsModel.create({
                title: t('toolbar.bpmnShortcutKeys'),
                showIcon: false,
                content: () => (<div class="event-listeners-box">
        ...
        <NPopover v-slots={{
                ...
                trigger: () => (<NButton onClick={mockSimulation}>
                <LucideIcon name="Bot" size={16}></LucideIcon>
              </NButton>)
            }}></NPopover>
        <NPopover v-slots={{
                default: () => t('toolbar.bpmnEvents'),
                trigger: () => (<NButton onClick={openEventsModel}>
                <LucideIcon name="Podcast" size={16}></LucideIcon>
              </NButton>)
            }}></NPopover>
        {minimapStatus.value && (<NPopover v-slots={{
                    default: () => t('toolbar.toggleMinimap'),
                    trigger: () => (<NButton onClick={() => minimapToggle()}>
                  <LucideIcon name="Map" size={16}></LucideIcon>
                </NButton>)
                }}></NPopover>)}
        {lintEnable.value && (<NPopover v-slots={{
                    default: () => t('toolbar.toggleProcessLint'),
                    trigger: () => (<NButton onClick={() => lintToggle()}>
                  <LucideIcon name="FileCheck" size={16}></LucideIcon>
                </NButton>)
                }}></NPopover>)}
        {shortcutKeysEnable.value && (<NPopover v-slots={{
                    default: () => t('toolbar.bpmnShortcutKeys'),
                    trigger: () => (<NButton onClick={() => openShortcutKeysModel()}>
