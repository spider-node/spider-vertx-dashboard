...
import { NForm, NFormItem, NInput, NRadioGroup, NRadio, NSwitch, NModal, NColorPicker, NInputNumber, NCard } from 'naive-ui';
...
const Setting = defineComponent({
    ...
    setup(props) {
        ...
        return () => (<div class="setting" onClick={(e) => e.stopPropagation()}>
        ...
        <NModal v-model:show={[modelVisible.value, 'show']}>
          <div class="settings-model">
            <NCard title={t('configForm.preferences')} style={{ overflow: 'hidden', height: '100%' }} size="small" contentStyle={{ flex: 1, overflowY: 'auto' }} segmented={{ content: true, footer: true }} v-slots={{
                default: () => (<NForm labelAlign="right" size="small" labelPlacement="left" labelWidth={140}>
                    ...
                    <NFormItem label={t('configForm.contextmenu')} feedback={t('configForm.there_are_different_states_under_TemplateChooser')}>
                    ...
                    {editorSettings.value.rendererMode === 'rewrite' && (<NFormItem label={t('configForm.customTheme')} class="theme-list" labelAlign="left" labelPlacement="top">
                        {themeColorKeys.map((key) => {
                            return (<div class="theme-item">
                              <div class="theme-item_label">{key}：</div>
                              <NColorPicker modes={['hex']} showAlpha={false} v-model={[editorSettings.value.customTheme[key], 'value']}></NColorPicker>
                            </div>);
                        })}
                        {themeOpacityKeys.map((key) => {
                            return (<div class="theme-item">
                              <div class="theme-item_label">{key}：</div>
                              <NInputNumber v-model={[editorSettings.value.customTheme[key], 'value']}></NInputNumber>
                            </div>);
                        })}
                      </NFormItem>)}
                  </NForm>),
                footer: () => (<div class="tips-message">
                    <div class="grip-tips">
                      <p>注意事项</p>
