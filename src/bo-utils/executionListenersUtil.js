...
import { getExtensionElementsList, addExtensionElements, removeExtensionElements } from '@/utils/BpmnExtensionElementsUtil';
...
export function getExecutionListenerType(listener) {
    ...
    if (isAny(listener, [`${prefix}:ExecutionListener`])) {
        if (listener.get(`${prefix}:class`))
            return 'class';
        if (listener.get(`${prefix}:expression`))
            return 'expression';
