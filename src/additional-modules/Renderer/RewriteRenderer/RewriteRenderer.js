...
import { append as svgAppend, attr as svgAttr, classes as svgClasses, create as svgCreate, select as svgSelect } from 'tiny-svg';
import Ids from 'ids';
...
import { getCirclePath, getDi, getDiamondPath, getFillColor, getLabelColor, getRectPath, getRoundRectPath, getSemantic, getStrokeColor, isCollection, isThrowEvent, isTypedEvent } from 'bpmn-js/lib/draw/BpmnRenderUtil.js';
...
const DEFAULT_FILL_OPACITY = 0.95, HIGH_FILL_OPACITY = 0.35;
...
class RewriteRenderer extends BaseRenderer {
    ...
    constructor(config, eventBus, styles, pathMap, canvas, textRenderer, elementRegistry, interactionEvents, priority) {
        ...
        const { defaultFillColor, defaultStartEventColor, defaultEndEventColor, defaultIntermediateEventColor, defaultIntermediateThrowEventColor, defaultIntermediateCatchEventColor, defaultTaskColor, defaultLabelColor, defaultGatewayColor, defaultSequenceColor } = { ...presetColor, ...config };
        const { defaultStartEventOpacity, defaultEndEventOpacity, defaultIntermediateThrowEventOpacity, defaultIntermediateCatchEventOpacity, defaultTaskOpacity, defaultLabelOpacity, defaultGatewayOpacity, defaultSequenceOpacity } = { ...presetOpacity, ...config };
        ...
        function attachTaskMarkers(parentGfx, element, taskMarkers) {
            ...
            const loopCharacteristics = obj.loopCharacteristics, isSequential = loopCharacteristics && loopCharacteristics.isSequential;
        ...
        const handlers = (this.handlers = {
            ...
            'bpmn:SequenceFlow': function (parentGfx, element) {
                ...
                const fill = getFillColor(element, defaultFillColor), stroke = getStrokeColor(element, defaultSequenceColor);
                const attrs = {
            ...
            'bpmn:Association': function (parentGfx, element, attrs) {
                const semantic = getSemantic(element);
                const fill = getFillColor(element, defaultFillColor), stroke = getStrokeColor(element, defaultTaskColor);
                attrs = assign({
            ...
            'bpmn:MessageFlow': function (parentGfx, element) {
                const semantic = getSemantic(element), di = getDi(element);
                const fill = getFillColor(element, defaultFillColor), stroke = getStrokeColor(element, defaultTaskColor);
                const pathData = createPathFromConnection(element);
                const attrs = {
                ...
                if (semantic.messageRef) {
                    ...
                    const messageBounds = message.getBBox(), labelBounds = label.getBBox();
                    const translateX = midPoint.x - labelBounds.width / 2, translateY = midPoint.y + messageBounds.height / 2 + ELEMENT_LABEL_DISTANCE;
            ...
            'bpmn:BoundaryEvent': function (parentGfx, element) {
                const semantic = getSemantic(element), cancel = semantic.cancelActivity;
                const attrs = {
                    strokeWidth: 1,
                    fill: getFillColor(element, defaultFillColor),
                    stroke: getStrokeColor(element, defaultTaskColor)
            ...
            ParallelMarker: function (parentGfx, element, position) {
                const markerPath = pathMap.getScaledPath('MARKER_PARALLEL', {
                    ...
                    containerHeight: element.height,
                    position: {
                        mx: (element.width / 2 + position.parallel) /
                            element.width,
                        my: (element.height - 20) / element.height
                    }
                });
            ...
            SequentialMarker: function (parentGfx, element, position) {
                const markerPath = pathMap.getScaledPath('MARKER_SEQUENTIAL', {
                    ...
                    containerHeight: element.height,
                    position: {
                        mx: (element.width / 2 + position.seq) /
                            element.width,
            ...
            CompensationMarker: function (parentGfx, element, position) {
                const markerMath = pathMap.getScaledPath('MARKER_COMPENSATION', {
                    ...
                    containerHeight: element.height,
                    position: {
                        mx: (element.width / 2 + position.compensation) /
                            element.width,
            ...
            LoopMarker: function (parentGfx, element, position) {
                const markerPath = pathMap.getScaledPath('MARKER_LOOP', {
                    ...
                    containerHeight: element.height,
                    position: {
                        mx: (element.width / 2 + position.loop) /
                            element.width,
            ...
            AdhocMarker: function (parentGfx, element, position) {
                const markerPath = pathMap.getScaledPath('MARKER_ADHOC', {
                    ...
                    containerHeight: element.height,
                    position: {
                        mx: (element.width / 2 + position.adhoc) /
                            element.width,
                        my: (element.height - 15) / element.height
                    }
                });
