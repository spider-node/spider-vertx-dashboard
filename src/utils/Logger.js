...
const types = ['primary', 'success', 'warn', 'error', 'info'];
function Logger() { }
...
Logger.print = function (type = 'default', text, back = false) {
    ...
    if (back) {
        ...
        console.log(`%c ${text} `, `border: 1px solid ${Logger.typeColor(type)};
...
Logger.pretty = function (type = 'primary', title, text) {
    if (typeof text === 'object') {
        ...
        console.log(`%c ${title}`, `background:${Logger.typeColor(type)};border:1px solid ${Logger.typeColor(type)};
        padding: 1px; border-radius: 4px; color: #fff;`);
        isArray(text) ? console.table(text) : console.dir(text);
        console.groupEnd();
        return;
    }
    console.log(`%c ${title} %c ${text} %c`, `background:${Logger.typeColor(type)};border:1px solid ${Logger.typeColor(type)};
      padding: 1px; border-radius: 4px 0 0 4px; color: #fff;`, `border:1px solid ${Logger.typeColor(type)};
      padding: 1px; border-radius: 0 4px 4px 0; color: ${Logger.typeColor(type)};`, 'background:transparent');
};
